...
export const Button = ({ children, onClick, variant = 'primary', className = '' }) => {
  const baseStyles = 'w-full py-4 px-6 rounded-[24px] font-bold text-lg transition-all active:scale-[0.97] ease-out-ios flex items-center justify-center gap-2';
  const variants = {
      ...
      <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[9999] p-4 animate-in fade-in duration-200">
        <div className="bg-[#1C1C1E] border border-white/10 w-full max-w-sm rounded-[32px] p-8 text-center space-y-6 shadow-2xl animate-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-2 gap-3 pt-2">
            <button 
              onClick={onCancel} 
              className="bg-white/5 text-white font-bold py-4 rounded-2xl text-sm hover:bg-white/10 transition-all active:scale-[0.97] ease-out-ios"
            >
              {cancelText}
            </button>
            <button 
              onClick={onConfirm} 
              className={`${isDanger ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-white text-black hover:bg-neutral-200'} font-bold py-4 rounded-2xl text-sm transition-all active:scale-[0.97] ease-out-ios shadow-lg`}
            >
              {confirmText}
            </button>
          </div>
